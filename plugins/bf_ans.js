let handler = async (m, { conn, text }) => {
  ...
  try {
    ...
    if(user === m.sender) return conn.reply(m.chat, `*Tidak bisa mempunyai bestie dengan diri sendiri.*`, m)
    if(user === conn.user.jid) return conn.reply(m.chat, `*Tidak bisa mempunyai bestie dengan bot.*`, m)
    ...
    if(global.db.data.users[user].pasangan != m.sender){
      ...
      conn.reply(m.chat,`*Selamat anda resmi mempunyai bestie dengan @${user.split('@')[0]}*\n\n*Semoga langgeng dan bahagia selalu @${user.split('@')[0]} â™¡ @${m.sender.split('@')[0]} ðŸ¥³ðŸ¥³*`,m,{contextInfo: {
